...
const starts = async (Lexxy = new WAConnection()) => {
    ...
    fs.existsSync('./Agnostic.json') && Lexxy.loadAuthInfo('./Agnostic.json')
    Lexxy.on('connecting', () => {
        start('2', 'Connecting...')
    })
    Lexxy.on('open', () => {
    ...
    await Lexxy.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./Agnostic.json', JSON.stringify(Lexxy.base64EncodedAuthInfo(), null, '\t'))
...
console.log('[ ! ]', `'${module}'`, 'DI Pantau Oleh HmdanDT')
